function qp=predict(model,x,y)
%
% Compute the predictive distribution 
% 1) for the latent function values f=g(X,w), e.g. f=X*w in case of a linear model
% 2) the observations y ~ p(y|f,h), where h are optional hyperparameters
%
% Note: this wrapper could be combined with predictive distribution.m
% 
% Pasi Jylänki 2014
...
% model-specific approximation for f
qp=model.fh_qf_pred(model,x);

% the predictive distribution for y ~ p(y|f,h,x)q(f)q(h)
qp=predictive_distribution(qp,y,model.lik.type,model.quad_opt);
